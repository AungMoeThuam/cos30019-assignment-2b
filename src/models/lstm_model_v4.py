#!/usr/bin/env python3
"""
LSTM traffic-flow model for TBRGS — v4.
...
def build_lstm_model(window_size: int, n_numeric: int,
                     n_movements: int, embedding_dim: int = EMBEDDING_DIM) -> Model:
    """Two-input LSTM: numeric sequence + movement_id embedding."""
...
def train_lstm(
    ...
    """Train LSTM v4 with movement_id embedding + scats_number."""
    ...
    # movement_id label encoding (integer only — Embedding handles the rest)
    ...
    model = build_lstm_model(window_size, len(numeric_cols), n_movements, EMBEDDING_DIM)

    print("Training LSTM v4 model...")
    ...
    model.fit(
        [X_num_tr, X_mov_tr], y_tr,
        epochs=75, batch_size=64, validation_split=0.1, shuffle=False,
    )
...
def predict_for_edges(
    ...
    """Predict traffic volume on edges using the trained LSTM v4 model."""
    ...
    for _, edge in edges_df.iterrows():
        ...
        # Find actual historical rows at target hour/dayofweek
        target_mask = (
        ...
        if target_positions and target_positions[-1] >= window_size:
            ...
            # Inject time context into last step
...
def main():
    parser = argparse.ArgumentParser(description="LSTM v4 — embedding + scats_number.")
    ...
    parser.add_argument("--model",    default="models_saved/lstm_model_v4.keras")
    parser.add_argument("--out",      default="models_saved/lstm_model_v4.keras")
    parser.add_argument("--metadata", default="models_saved/lstm_metadata_v4.joblib")
    parser.add_argument("--metrics",  default="models_saved/lstm_metrics_v4.csv")
    parser.add_argument("--time",     default="1100")
    parser.add_argument("--dayofweek", type=int, default=None)
    parser.add_argument("--out_csv",  default="data/processed/lstm_edge_predictions_v4.csv")
