...
# This module defines the LSTM (Long Short-Term Memory) neural network architecture using TensorFlow/Keras.
...
class LSTMRegressor(tf.keras.Model):
    ...
    #    - hidden_units: Number of units/neurons in the LSTM layer (e.g., 64).
    #    - output_dim: Number of predicted values (typically 1 for next flow value).
    #    - Initialize layers:
    #      - self.lstm_layer = layers.LSTM(hidden_units, return_sequences=False)
    ...
    #    - Pass input x (shape: [batch_size, seq_len, num_features]) through self.lstm_layer.
    #    - Pass the output of the LSTM layer through self.dense_layer.
